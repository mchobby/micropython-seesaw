""" Retro portage of Adafruit seesaw to MicroPython

An I2C to whatever helper chip.

Author(s): 
 - Dean Miller (CircuitPython version)
 - Meurisse Dominique (MicroPython retro-portage)
...
__version__ = "0.0.1"
__repo__ = "https://github.com/mchobby/micropython-seesaw.git"
...
class Seesaw:
    ...
    def __init__(self, i2c, addr=0x49, drdy=None, reset=True):
        self._drdy = drdy
        if drdy is not None:
            drdy.init( Pin.IN, Pin.PULL_UP ) # switch_to_input()
        ...
        if pid == _CRICKIT_PID:
            from .crickit import Crickit_Pinmap

            self.pin_mapping = Crickit_Pinmap
        elif pid == _ROBOHATMM1_PID:
            from .robohat import MM1_Pinmap
            ...
            from .attinyx16 import ATtinyx16_Pinmap

            self.pin_mapping = ATtinyx16_Pinmap
        elif self.chip_id == _SAMD09_HW_ID_CODE:
            from .samd09 import SAMD09_Pinmap
            ...
            from .attiny8x7 import ATtiny8x7_Pinmap
    ...
    def read(self, reg_base, reg, buf, delay=0.008):
        ...
        self.i2c.writeto( self.addr, bytes([reg_base,reg]) ) # Stop_condition=False
